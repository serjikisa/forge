...
- **Minimal dependencies** — only `golang.org/x/term` for terminal handling, no other external deps
...
2. Implement the `Provider` interface:
   ...
   ```
3. Optionally implement `ModelSwitcher` for runtime model switching
4. Map `ChatRequest` to the provider's API format
5. Parse streaming responses into `ChatEvent` channel
6. Add a case in `cmd/forge/flags.go` `newProvider()`
7. Add config fields to `config.go`
8. Add tests using `httptest`
9. Document in `docs/providers.md`
...
2. Implement the `Tool` interface:
   ...
   ```
3. Register in `tool.Registry()` in `tool.go`
4. Add tests
