...
// Optional interfaces:
type ModelSwitcher interface { SetModel(model string) }
type ModelInfo interface { ParameterSize() string }
```
...
| Feature | Ollama | OpenAI | DeepSeek | Anthropic | Bedrock |
|---------|--------|--------|----------|-----------|---------|
| Auth | None | Bearer token | Bearer token | x-api-key | AWS SigV4 |
| Streaming | NDJSON | SSE | SSE | SSE | Event stream |
| Tool calling | Varies by model | Full | Partial (V3 only) | Full | Full |
| System prompt | In messages | In messages | In messages | Top-level field | Top-level array |
| Tool definition key | `function.parameters` | `function.parameters` | `function.parameters` | `input_schema` | `toolSpec.inputSchema.json` |
| Tool args format | Parsed object | Streamed JSON string | Streamed JSON string | Streamed JSON delta | Event stream |
| Rate limits | None (local) | Per-tier | Aggressive | Per-model | Per-account |
