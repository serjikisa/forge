...
- web_search: Search the web (only included when enabled in config)

Use tools when you need to inspect or modify the user's project.
Read files before making claims about their contents.
```

Note: The capabilities block is assembled dynamically. `web_search` is only included when `search_provider` is configured. The tool list is generated from the registered tools, not hardcoded.
...
Token estimation: ~4 characters per token (rough heuristic). No external tokenizer needed for estimation — exact counts come from the API response when available.
