...
func runChat(ctx context.Context, cfg *config.Config) {
	...
	tools := tool.Registry(tool.RegistryOptions{ShellTimeout: cfg.ShellTimeout})
	ui := tui.New(prov, model)
	defer ui.Restore()
	a := agent.New(p, tools, ui, model, agent.Options{
...
func runAsk(ctx context.Context, cfg *config.Config) {
	...
	if sprompt, ok := cfg.ModelPrompts[model]; ok {
		a.SetSystemPrompt(sprompt)
	}
	if sp := resolveSystemPrompt(); sp != "" {
		a.SetSystemPrompt(sp)
	}
