...
type Options struct {
	MaxConcurrency int
	ShellTimeout   int // seconds; 0 means default (30)
}

func New(p provider.Provider, tools []tool.Tool, ui tui.UI, model string, opts ...Options) *Agent {
...
// isSmallModel checks if the model has <= 8B parameters.
	...
	for {
		...
		case input == "/save":
			if err := a.SaveSession(); err != nil {
				...
				a.tui.Info("session saved")
			}
			continue
		case input == "/resume":
			path, err := LatestSession()
			if err != nil {
				a.tui.Error(err.Error())
				continue
			}
			...
			a.tui.Info(fmt.Sprintf("resumed session (%d messages)", len(sess.Messages)))
			continue
		case input == "/model":
...
func (a *Agent) consumeStream(events <-chan provider.ChatEvent) (string, []provider.ToolCall) {
	...
	// Buffer initial tokens to detect JSON tool calls before streaming
	const bufferThreshold = 20
	var initialBuf strings.Builder
	bufferFlushed := false
	isJSON := false
	...
	for ev := range events {
		...
		case provider.EventText:
			...
			if a.noTools {
				...
				// Buffer a small amount to detect if response is JSON tool call
				if !bufferFlushed {
					initialBuf.WriteString(ev.Text)
					trimmed := strings.TrimSpace(initialBuf.String())
					if len(trimmed) > 0 && trimmed[0] == '{' {
						// Looks like JSON tool call — buffer everything
						isJSON = true
						continue
					}
					if initialBuf.Len() >= bufferThreshold {
						// Not JSON — flush buffer and stream
						bufferFlushed = true
						if !streaming {
							a.tui.StreamToken("  ")
							streaming = true
						}
						a.tui.StreamToken(initialBuf.String())
					}
				} else {
					a.tui.StreamToken(ev.Text)
				}
	...
	// Flush any remaining buffered text that wasn't streamed
	if !a.noTools && !isJSON && !bufferFlushed && initialBuf.Len() > 0 {
		if !streaming {
			a.tui.StreamToken("  ")
			streaming = true
		}
	...
	// Handle text when native tool calls were received (strip echoed JSON)
	if !a.noTools && len(toolCalls) > 0 && text.Len() > 0 {
		output := text.String()
		knownTools := make(map[string]bool, len(a.toolMap))
		for name := range a.toolMap {
...
func toolDetail(name string, args json.RawMessage, result string) string {
	...
	case "edit_file":
		if p, ok := m["path"]; ok {
			return shortName(fmt.Sprintf("%v", p))
		}
		return ""
	case "list_directory":
...
// estimateTokens approximates token count: code-heavy content averages ~3 chars/token.
...
func (a *Agent) compactHistory() {
	...
	for i := len(a.history) - 1; i >= 1; i-- {
		budget -= estimateTokens(a.history[i].Content)
		if budget < 0 {
			break
		}
		start = i
...
func (a *Agent) showTokenBreakdown() {
	var system, history, input int
	lastUserIdx := -1
	for i := len(a.history) - 1; i >= 0; i-- {
		if a.history[i].Role == "user" {
			lastUserIdx = i
	...
	a.tui.Info(fmt.Sprintf("tokens: system=%d history=%d input=%d output=%d (total=%d)",
		system, history, input, a.outputTokens, system+history+input+a.outputTokens))
}
...
You have tools: read_file, write_file, edit_file, list_directory, shell_exec, search_code, web_search, web_fetch.
Always use your tools proactively. Never ask the user to paste code or provide information you can get yourself.
Use read_file and list_directory to explore code. Use edit_file for small changes (more efficient than rewriting). Use web_search to find information online. Use web_fetch to read web pages.
...
- edit_file: Use to make targeted edits — replace a specific string in a file. More efficient than rewriting the entire file for small changes.
...
- To edit a file: {"name": "edit_file", "arguments": {"path": "main.go", "old_string": "old code", "new_string": "new code"}}
